...
export default function TeacherDashboard() {
    ...
    const renderQrCode = (session) => {
        ...
        if (typeof qrValue === 'string' && qrValue.startsWith('data:image')) {
            return <img src={qrValue} alt={`QR for ${session.sessionId}`} className="w-40 h-40" />;
        }

        return <QRCode value={String(qrValue)} size={160} level="H" />;
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {activeSessions.length === 0 ? (
                        ...
                        activeSessions.map((session, i) => (
                            <motion.div key={session.sessionId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                                <Card className="flex flex-col h-full shadow-md hover:shadow-lg transition-shadow border-slate-200 overflow-hidden">
                                    <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <CardTitle className="text-xl text-slate-800">{session.subject}</CardTitle>
                                                <p className="text-xs text-slate-400 mt-1 font-mono truncate max-w-[200px]" title={session.sessionId}>{session.sessionId}</p>
                                            </div>
